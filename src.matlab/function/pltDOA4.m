function pltDOA4(time_in,time_border,wave_in,fs,fft_N,inc_data1,inc_data1ave)
% function pltDOA4(time_in,time_border,wave_in,fs,fft_N,inc_data1,inc_data1ave,obsDOA,inte_incl)
...
if false
    ...
    plot(time_in,wave_in(:,2)/max(abs(wave_in(:,2))),'Color',cm(2,:));hold on;
    plot(time_in,wave_in(:,3)/max(abs(wave_in(:,3))),'Color',cm(3,:));hold on;
...
%         ylim([0,200]);
    ...
    subplot(4,1,4)
    plot(time_border,inc_data1ave,'Color',cm(2,:));hold on;
%     plot(time_border,inte_incl,'Color',cm(3,:));hold on;
%     line([0,dlength],[obsDOA,obsDOA],'Color',cm(2,:));
%     datetick('x',21,'keepticks');
%     datetick('x',21);
    xlim([t1 t2]);
    hold on;
