function [Data1,Data3,lamda,n_smooth]=clcSPMmtrx_ave_cpsd(fft_N,wave,freq_C,deltaF,fs)
...
Sxx=cpsd(wave_x,wave_x,[],[],fft_N,fs);
Syy=cpsd(wave_y,wave_y,[],[],fft_N,fs);
Szz=cpsd(wave_z,wave_z,[],[],fft_N,fs);
Sxy=cpsd(wave_x,wave_y,[],[],fft_N,fs);   
Sxz=cpsd(wave_x,wave_z,[],[],fft_N,fs);
Syx=cpsd(wave_y,wave_x,[],[],fft_N,fs);
Syz=cpsd(wave_y,wave_z,[],[],fft_N,fs);
Szx=cpsd(wave_z,wave_x,[],[],fft_N,fs);  
Szy=cpsd(wave_z,wave_y,[],[],fft_N,fs);
